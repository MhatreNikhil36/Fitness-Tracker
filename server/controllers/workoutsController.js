...
export const getPastActivity = async (req, res) => {
  try {
    ...
    // Fetch completed workouts (both regular + AI-generated)
    const [workouts] = await db.query(`
      SELECT cw.id,
             COALESCE(wt.name, cw.custom_workout_name) AS workout_name,
             cw.workout_date AS completed_on,
             cw.calories_burned AS total_calories_burned
      FROM completedworkouts cw
      LEFT JOIN workouttemplates wt ON cw.template_id = wt.id
      WHERE cw.user_id = ?
    `, [userId]);
...
export const completeWorkout = async (req, res) => {
  try {
    const userId = req.userId;
    const { workout_id, workout_name, duration_minutes, calories_burned } = req.body;
    ...
    if (workout_id) {
      ...
      await db.query(
        `INSERT INTO completedworkouts (user_id, custom_workout_name, workout_date, duration, calories_burned)
         VALUES (?, ?, CURDATE(), ?, ?)`,
        [userId, workout_name ?? "AI-Recommended Workout", duration_minutes ?? 30, calories_burned ?? 200]
      );

      return res.status(200).json({ message: "AI-generated workout logged successfully" });
    }
  } catch (err) {
