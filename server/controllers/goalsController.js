...
export const createGoal = async (req, res) => {
  ...
  try {
    ...
    const [createdRows] = await pool.query("SELECT * FROM goals WHERE id = ?", [
      result.insertId,
    ]);
...
export const updateGoal = async (req, res) => {
  ...
  const { id, goal_type, target_value, current_value, status, deadline } =
    req.body;
...
export const deleteGoal = async (req, res) => {
  ...
  try {
    ...
    await pool.query("DELETE FROM goals WHERE id = ? AND user_id = ?", [
      goalId,
      userId,
    ]);
